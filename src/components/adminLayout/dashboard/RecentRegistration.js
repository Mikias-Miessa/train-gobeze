...
import {Table,CircularProgress,TableBody,TableCell,TableHead,TableRow, Box, Typography ,Popper, Fade,Paper,Button,IconButton, Backdrop} from '@mui/material';

import Title from '../../Title';
import {getStudents,reset,deleteStudent} from '../../../../store/studentSlice'
  ...
  return (
    <>
      ...
      loading ?<TableRow><TableCell colSpan={5}><CircularProgress color='primary' sx={{m:'auto'}} /> </TableCell></TableRow> : 
          students.filter(student => !student.contacted).map((student,index) => student &&(
            <TableRow key={index}>
...
<Button   sx={{border: 'none',color: 'secondary.main', fontWeight: '300', textTransform: 'none'}} variant="outlined" startIcon={<DomainVerificationIcon fontSize='small' />}
